      ...
      <b-col class="px-0 mx-0">
        ...
        >
          ...
          <b-row>
            ...
            <b-col cols="3">
              <b-button size="sm" variant="info" @click="showDialogue">
                select crop
              </b-button>
            </b-col>
          </b-row>
        ...
        >
          ...
          <b-row>
            ...
            <b-col cols="6">
              {{ qaScore[pageIdComputed][qaScore[pageIdComputed].length - 1].value }} /
              {{ 10 * qaList.length }}
            </b-col>
          </b-row>
          ...
          <div v-show="index===0" class="mb-2">
            ...
            <div class="font-weight-bold">
              Target nutrient: <span
                class="text-danger"
              >{{ myFamilyWatcher.keyNutrient }}</span>
            </div>
            <div class="font-weight-bold">
              Daily Target:
...
import { getNutritionDemandList, getNutritionSupplyList, getProductionTarget, validateMyFamily } from '@/plugins/helper'
...
export default {
  ...
  methods: {
    ...
    onAnsChanged (dat) {
      // 作業用のmyFamilyコピー作成
      const res = JSON.parse(JSON.stringify(this.myFamilyWatcher))
      // 更新されたmenuを入れ替える
