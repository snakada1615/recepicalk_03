<template>
  <b-container>
    <b-row class="my-2">
      ...
      <b-col cols="12" lg="6">
        <b-card
          ...
          <recepi-table
            :items.sync="myAppWatcher.familyCases.find((item) => item.name === currentFamily).menuCases[pageIdComputed].menu"
    ...
    <fctTableModal2
      ...
      :menu-cases.sync="myAppWatcher.familyCases.find((item) => item.name === currentFamily).menuCases[pageIdComputed].menu"
...
export default {
  ...
  computed: {
    ...
    diversityStatus() {
      const vm = this
      return vm.myAppWatcher.familyCases.find((item) =>
        item.name === this.currentFamily).menuCases.map((foodsTemp) => {
  ...
  watch: {
    ...
    myApp: {
      ...
      handler(newVal) {
        ...
        this.pageMemo = this.myAppWatcher.familyCases.find((item) =>
          item.name === this.currentFamily).menuCases.map((item2) => {
          return item2.note
        })
        this.updatePfc()
      }
  ...
  methods: {
    ...
    nutritionDemandGetter() {
      const vm = this
      const res = vm.myAppWatcher.familyCases.find((item) =>
        item.name === this.currentFamily).member.reduce((accumulator, item, index) => {
      ...
      return [...Array(vm.maxPage)].map(() => res);
    },
    /**
    ...
    nutritionSupplyGetter2() {
      const vm = this
      return vm.myAppWatcher.familyCases.find((item) =>
        item.name === this.currentFamily).menuCases.map((datArray) => {
        if (datArray.menu.length > 0) {
          return datArray.menu.reduce((accumulator, item) => {
